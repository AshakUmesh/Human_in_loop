...
def RL_training():
    ...
    if args.resume and os.path.exists(log_dir):
        print(f'Loading checkpoint from {log_dir}...')
        checkpoint  = torch.load(log_dir, map_location=args.device)
        DRL.load(log_dir)
        start_epoch = checkpoint['epoch'] + 1
        print(f'Resumed from episode {start_epoch}')
        print(f'Training will continue from episode {start_epoch}')
    else:
        start_epoch = 0
        if args.resume:
            print(f'WARNING: No checkpoint found at {log_dir}')
            print('Starting from episode 0')
        else:
            print('Starting fresh training from episode 0')
    ...
    for i in range(start_epoch, int(args.maximum_episode)):
        ...
        while True:
            ...
            if done:
                ...
                episode_reward_list.append(mean_reward)
                episode_duration_list.append(step)
                intervene_percent.append(
                    len(intervene_percent_per_episode[i]))

                writer.add_scalar('reward/reward_episode',
                                  mean_reward, i)
                writer.add_scalar('reward/reward_episode_noshaping',
                                  np.mean(reward_e_record[i]), i)
                writer.add_scalar('reward/duration_episode',
                                  step, i)
                writer.add_scalar('percent_intervene',
                                  len(intervene_percent_per_episode[i]), i)
                writer.add_scalar('exploration_rate',
                                  round(exploration_rate, 4), i)
                writer.add_scalar('loss/loss_critic',
                                  round(np.average(c_loss), 4), i)
                writer.add_scalar('loss/loss_actor',
                                  round(np.average(a_loss), 4), i)

                # Print real episode number clearly
                # print(f'[Ep {i:4d}] '
                #       f'Reward: {mean_reward:+.3f} | '
                #       f'Steps: {step:4d} | '
                #       f'Intervene: '
                #       f'{len(intervene_percent_per_episode[i]):3d} | '
                #       f'TotalSteps: {total_step}')

                print(f'[GLOBAL Ep {i:4d}] '
                      f'Reward: {mean_reward:+.3f} | '
                      f'Steps: {step:4d} | '
                      f'Intervene: '
                      f'{len(intervene_percent_per_episode[i]):3d} | '
                ...
                DRL.save(log_dir, i)
                print(f'>>> Checkpoint saved episode {i}')

                # Save mat data every 10 episodes
                if i % 10 == 0 and i > 0:
                    scio.savemat(
                        'data{}_episode{}.mat'.format(
                            args.algorithm, i),
                        mdict={
                    ...
                    print(f'>>> Data mat saved episode {i}')
